...
const ModuleLottie = dynamic(() => import("@/components/ui/ModuleLottie"), {
  ssr: false,
  loading: () => <div className="w-12 h-12 bg-white/20 rounded-lg animate-pulse" />,
});
...
const humanOSMenu = {
  title: "Human OS",
  items: [
    { name: "Emotional Pattern Mapping", href: "/human-os", description: "Track recurring emotions & triggers", lottie: "/animations/modules/PersonalHistory.json" },
    { name: "Thought Reframe Engine", href: "/human-os", description: "CBT + NLP powered reframing", lottie: "/animations/modules/DrivingQuestion.json" },
    { name: "Nervous System Tracker", href: "/human-os", description: "Beyond HRV monitoring", lottie: "/animations/modules/Vagal.json" },
    { name: "Alignment Actions", href: "/human-os", description: "Actions that actually matter", lottie: "/animations/modules/SetOutcomes.json" },
    { name: "Identity Shift Engine", href: "/human-os", description: "Track your growth patterns", lottie: "/animations/modules/ElicitValues.json" },
  ],
};
...
const navigation = [
  ...
  { name: "Human OS", href: "#", hasDropdown: true, menu: "humanOS" },
        ...
        <div className="container py-8">
          ...
          {activeDropdown === "pillars" && (
            <div className="flex gap-8">
              {/* Pillar Cards - Glassmorphism with videos */}
              <div className="flex gap-4 flex-1">
                {pillarsMenu.items.map((pillar) => (
                  <Link
                    ...
                    className="flex-1 rounded-2xl overflow-hidden transition-all duration-200 hover:shadow-xl group relative h-48 bg-brand-navy/60 backdrop-blur-xl border border-white/20"
                  >
                    {/* Video Background */}
                    <video
                      src={pillar.video}
                    ...
                    <div className="relative h-full flex flex-col p-5">
                      <h3 className="font-semibold text-white text-lg mb-1">{pillar.name}</h3>
                      <p className="text-sm text-white/70 mb-4">{pillar.description}</p>
                      <div className="mt-auto flex items-center gap-2">
                        <ArrowRight className="h-5 w-5 p-1 rounded-full bg-white/20 text-white group-hover:bg-white group-hover:text-brand-navy transition-colors" />
                      </div>
                    </div>
                  </Link>
                ))}
          ...
          {/* Human OS Menu */}
          {activeDropdown === "humanOS" && (
            <div className="flex gap-8">
              {/* Human OS Feature Cards - Glassmorphism with Lotties */}
              <div className="grid grid-cols-3 gap-4 flex-1">
                {humanOSMenu.items.map((feature) => (
                  <Link
                    key={feature.name}
                    href={feature.href}
                    onClick={() => setActiveDropdown(null)}
                    className="rounded-2xl bg-brand-navy/60 backdrop-blur-xl border border-white/20 p-5 transition-all duration-200 hover:shadow-xl group relative overflow-hidden"
                  >
                    {/* Lottie Background */}
                    <div className="absolute top-2 right-2 w-16 h-16 opacity-60">
                      <ModuleLottie animationPath={feature.lottie} />
                    </div>
                    <div className="relative">
                      <h3 className="font-semibold text-white text-sm mb-2">{feature.name}</h3>
              ...
              <div className="w-48 space-y-4 pt-4">
                ...
                >
                  <ArrowRight className="h-5 w-5 p-1 rounded-full bg-palette-sea text-white" />
                  <span className="text-sm font-medium">Human OS</span>
                </Link>
              </div>
            </div>
          )}
                    ...
                    {activeDropdown === item.menu && item.menu === "pillars" && (
                      <div className="pl-4 space-y-1 mt-1 mb-2">
                        {pillarsMenu.items.map((pillar) => (
                          ...
                          >
                            <pillar.icon className="h-4 w-4 text-palette-sea" />
                            {pillar.name}
                          </Link>
                        ))}
                      </div>
                    )}
                    {activeDropdown === item.menu && item.menu === "humanOS" && (
                      <div className="pl-4 space-y-1 mt-1 mb-2">
                        {humanOSMenu.items.map((feature) => (
                          <Link
                            key={feature.name}
